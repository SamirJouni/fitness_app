...
def load_exercises():
    exercises_path = os.path.join(app.root_path, "exercises.json")
    ...
    if request.method == "POST":
        ...
        if "image" in request.files:
            ...
            if image:
                ...
                # Absoluter Pfad zum Speichern des neuen Bildes
                image_path = os.path.join(
                    app.root_path, "static", "images", image.filename
                )
        ...
        return redirect(
            url_for("edit_database")
        )  # Nach dem Speichern zurück zur Datenbankbearbeitung
...
def add_exercise():
    if request.method == "POST":
        new_exercise = {
            "id": len(load_exercises())
            + 1,  # Neue ID basierend auf der Anzahl der bestehenden Übungen
            "title": request.form["title"],
            "description": request.form["description"],
            "image": request.files[
                "image"
            ].filename,  # Hier wird der Dateiname gespeichert
...
def countdown():
    ...
    for exercise in exercises:
        exercise["image_url"] = url_for(
            "static", filename=os.path.join("images", exercise["image"])
        )  # Bild-URL hinzufügen
